...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        let fd_path_str = format!("/proc/{}/fd/{}", pid, fd);
        let link_path_buf = fs::read_link(fd_path_str).ok()?;
        // println!("{}", link_path_buf.to_str().unwrap()); // /dev/pts/7
        let name = OpenFile::path_to_name(link_path_buf.to_str().unwrap());

        let fdinfo_path_str = format!("/proc/{}/fdinfo/{}", pid, fd);
        let fdinfo_str = fs::read_to_string(fdinfo_path_str).ok()?;
        let cursor = OpenFile::parse_cursor(&fdinfo_str)?;
        let access_mode = OpenFile::parse_access_mode(&fdinfo_str)?;
